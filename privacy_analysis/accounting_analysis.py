...
def get_std(q = None, EPOCH = None, epsilon = None, delta = None, verbose = True):
    try:
        ...
        print(f'==>[error]: calculating std using approximate method, {e}')
        std_analyszer_1 = 100
    try:
        print('==> calculating std using analytical method')
        ...
        print(f'==>[error]: calculating std using analytical method, {e}')
        std_analyszer_2 = 100
    
    std = min(std_analyszer_1, std_analyszer_2)
    if verbose:
        print(f'\n==> analytical 1:{round(std_analyszer_1, 4)}, numerical 2:{round(std_analyszer_2,4)}')
